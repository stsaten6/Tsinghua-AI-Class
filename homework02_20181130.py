...
def plotData(X, y):
    like = np.where(y==1)                            # 通过使用 np.where 函数查找所有满足条件的数据，查找所有满足标志 y == 1 的数据，并赋值给 pos
    unlike = np.where(y==0)                            # 通过使用 np.where 函数查找所有满足条件的数据，查找所有满足标志 y == 0 的数据，并赋值给 neg
    # 通过使用 plt.plot 函数作图，对所有满足标志 y == 1 的数据作图，点采用 s (正方形)，代表 square, 点的大小为 7 单位，颜色为 红色 red
    p1 = plt.plot(X[like, 0], X[like, 1], marker='s', markersize=7, color='red')[0]
    # 请补全 通过使用 plt.plot 函数作图，对所有满足标志 y == 0 的数据作图，点采用 o (圆形)，代表 circle, 点的大小为 7 单位，颜色为 绿色 green 
    p2 = plt.plot(X[unlike, 0], X[unlike, 1], marker='o', markersize=7, color='green')[0]

    return p1, p2                                   # 返回作图操作plt, p1, p2

# normalization： 定义一个 normalization 函数，输入参数是原始数据 X ，返回归一化后的数据 X_norm ， 目的是为了数据预处理，得到归一化后的数据 X_norm
def normalization(X):
    Xmin = X.min(0)#请补全                                         # 请补全 通过使用 np.min 函数，计算原始数据沿着 axis=0 方向的最小值，即：求每一列的最小值，并赋值给 Xmin
    Xmax = X.max(0)#请补全                                         # 请补全 通过使用 np.max 函数，计算原始数据沿着 axis=0 方向的最大值，即：求每一列的最大值，并赋值给 Xmax
    Xmu  = X.mean(0) #请补全                                         # 请补全 通过使用 np.mean 函数，计算原始数据均值，并赋值给 Xmu                 
    X_norm = (X-Xmu)/(Xmax-Xmin) #请补全                                      # 请补全 计算归一化后的数据，归一化公式为：(X-Xmu)/(Xmax-Xmin)，归一化后数据范围为 [-1,1]
    #print(Xmin,Xmax,Xmu,X_norm)
...
def loss(X, Y, w):                                  # 定义一个 损失函数 loss 函数 （loss function），输入参数是 X, Y, w， 返回的是 损失函数的值
    ...
    for i in range(m):                              # 通过 for 循环结构，遍历数据集 Y_ 所有的行，其中，i 可取的数依次为：0，1 ，2，....，m-1， 数据集 Y_ 总共有 m 行    
        ...
        #print(Y[i]*np.log(Y_[i])+(1-Y[i])*np.log(1-Y_[i]))                    #for test
